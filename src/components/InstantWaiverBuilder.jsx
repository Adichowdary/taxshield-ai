...
Executive Consumer & TaxShield Member`
  ...
  return (
    <section id="waiver-builder" className="py-20 md:py-28 relative border-t border-white/[0.08] bg-[#050811]/90 backdrop-blur-xl">
      <Container>
        <SectionHeader
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 max-w-5xl mx-auto items-start">
          ...
          {/* Controls Form */}
          <div className="lg:col-span-4 vault-glass border border-white/10 rounded-2xl p-6 space-y-4 shadow-xl">
            <div className="flex items-center gap-2 pb-3 border-b border-white/[0.08]">
              <FileSignature size={18} className="text-[#D4AF37]" />
              <h3 className="font-bold text-sm text-white font-poppins">Notice Parameters</h3>
            </div>

            <div className="space-y-3.5 text-xs">
              <div>
                <label className="block text-slate-300 mb-1.5 font-medium">Establishment / Venue</label>
                <input
                  type="text"
                  value={restaurantName}
                  onChange={(e) => setRestaurantName(e.target.value)}
                  className="w-full px-3.5 py-2.5 rounded-xl bg-black/50 border border-white/10 text-white text-xs focus:outline-none focus:border-[#D4AF37] transition-colors"
                />
              </div>

              <div>
                <label className="block text-slate-300 mb-1.5 font-medium">Disputed Service Charge (₹)</label>
                <input
                  type="number"
                  value={serviceChargeAmount}
                  onChange={(e) => setServiceChargeAmount(e.target.value)}
                  className="w-full px-3.5 py-2.5 rounded-xl bg-black/50 border border-white/10 text-white text-xs focus:outline-none focus:border-[#D4AF37] font-mono transition-colors"
                />
              </div>
            </div>
            ...
            <div className="pt-2">
              <button
                onClick={handleCopy}
                className="w-full flex items-center justify-center gap-2 py-3 px-4 rounded-xl auth-cta text-slate-950 text-xs font-bold tracking-wide transition-all shadow-lg shadow-[#D4AF37]/20 cursor-pointer"
              >
            ...
            <div className="p-3 bg-white/[0.03] rounded-xl border border-white/[0.06] text-[11px] text-slate-300 flex items-start gap-2">
          ...
          <div className="lg:col-span-8 vault-glass border border-white/10 rounded-2xl p-6 sm:p-8 shadow-2xl relative overflow-hidden">
            ...
            <div className="flex items-center justify-between pb-4 border-b border-white/[0.08] mb-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-mono text-slate-200 font-bold">ccpa_statutory_dispute_memo.txt</span>
              </div>
              <span className="text-[10px] font-mono uppercase text-[#FDE68A] tracking-wider font-bold">
                Format: Executive Notice
              </span>
            </div>

            <pre className="text-xs font-mono text-slate-300 leading-relaxed whitespace-pre-wrap select-all bg-black/60 p-5 rounded-xl border border-white/[0.06] max-h-[380px] overflow-y-auto">
              {letterText}
            </pre>
          </div>
