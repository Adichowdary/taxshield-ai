...
// Fallback high-class metallic gold and emerald gradient image data URL if src is not specified
const DEFAULT_GOLD_EMERALD_TEXTURE = `data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="800" height="400" viewBox="0 0 800 400"><defs><linearGradient id="g" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="%23FFF0C2"/><stop offset="30%" stop-color="%23D4AF37"/><stop offset="70%" stop-color="%2310B981"/><stop offset="100%" stop-color="%23059669"/></linearGradient><radialGradient id="r" cx="50%" cy="50%" r="50%"><stop offset="0%" stop-color="%23F59E0B" stop-opacity="0.8"/><stop offset="100%" stop-color="%23022C22" stop-opacity="0.9"/></radialGradient></defs><rect width="800" height="400" fill="url(%23g)"/><rect width="800" height="400" fill="url(%23r)" style="mix-blend-mode:overlay"/><path d="M0 100 Q 200 300 400 100 T 800 100" stroke="%23FFFFFF" stroke-width="4" fill="none" opacity="0.3"/><path d="M0 300 Q 400 0 800 300" stroke="%23D4AF37" stroke-width="6" fill="none" opacity="0.4"/></svg>`
...
const MaskedHeading = ({
  ...
  const mediaSource = src || DEFAULT_GOLD_EMERALD_TEXTURE
      ...
      <span ref={revealRef} className="masked-heading__reveal">
        <span className="masked-heading__clip" style={{ clipPath: `url(#${clipId})` }}>
          <span ref={mediaRef} className="masked-heading__media">
            {mediaType === 'video' ? (
              <video className="masked-heading__source" src={mediaSource} poster={poster} autoPlay muted loop playsInline />
            ) : (
              <img className="masked-heading__source" src={mediaSource} alt="" draggable={false} />
            )}
          </span>
        </span>
      </span>
