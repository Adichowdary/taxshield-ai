...
export default function InteractiveBillSimulator() {
  ...
  return (
    <section id="bill-simulator" className="py-20 md:py-28 relative border-t border-white/[0.08] bg-[#060912]/80 backdrop-blur-xl">
      <Container>
        <SectionHeader
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2.5 px-4 py-2.5 rounded-xl text-xs font-semibold tracking-wide transition-all cursor-pointer ${
                  isSelected
                    ? 'bg-[#D4AF37] text-slate-950 shadow-lg shadow-[#D4AF37]/25 font-bold border border-[#FDE68A]/60'
                    : 'vault-glass text-slate-300 hover:text-white hover:border-[#D4AF37]/35 border border-white/[0.08]'
                }`}
              >
                <Icon size={14} className={isSelected ? 'text-slate-950' : 'text-[#D4AF37]'} />
                <span>{s.name}</span>
                <span className={`text-[10px] ${isSelected ? 'text-slate-800' : 'text-slate-400'}`}>({s.type})</span>
              </button>
            )
          })}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 max-w-6xl mx-auto items-start">
          ...
          <div className="lg:col-span-7 vault-glass border border-white/10 rounded-2xl p-6 sm:p-8 shadow-2xl relative overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-5 border-b border-white/[0.08]">
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="font-bold text-lg text-white tracking-tight font-poppins">{scenario.name}</h3>
                ...
                <p className="text-xs text-slate-400 mt-0.5 font-mono">GSTIN: {scenario.merchantGstReg} • {scenario.date}</p>
              </div>
              <div className="text-right">
                <span className="text-[10px] uppercase font-mono tracking-wider text-slate-400 block">Bill Audit ID</span>
                <span className="text-xs font-mono font-bold text-[#FDE68A]">#TX-2026-{scenario.id.toUpperCase().slice(0, 4)}</span>
              </div>
            </div>

            {/* Line Items Table */}
            <div className="py-5 space-y-2.5">
              <span className="text-[10px] uppercase font-mono tracking-wider text-slate-400">Audited Line Items</span>
              <div className="divide-y divide-white/[0.04]">
                {scenario.items.map((item, idx) => (
                  <div key={idx} className="py-2.5 flex items-center justify-between text-xs">
                    <span className="text-slate-200 font-medium">{item.name}</span>
                    <span className="font-mono text-white font-semibold">₹{item.price.toLocaleString('en-IN')}.00</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="pt-4 border-t border-white/[0.08] space-y-2.5 font-mono text-xs">
              <div className="flex justify-between text-slate-400">
                <span>Food & Beverage Subtotal</span>
                <span className="text-white font-semibold">₹{scenario.subtotal.toLocaleString('en-IN')}.00</span>
              </div>
              ...
              <div className={`flex justify-between items-center py-2 px-3 rounded-xl transition-colors ${
                ...
                <div className="flex items-center gap-2 font-sans">
                  ...
                  <span className={`text-[10px] px-2 py-0.5 rounded font-mono font-bold ${
                    waiveServiceCharge ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30' : 'bg-rose-500/20 text-rose-300'
                  }`}>
              ...
              <div className="pt-4 border-t border-white/[0.12] flex justify-between items-baseline font-bold text-sm">
                <span className="text-white font-sans text-base">Authorized Net Payable</span>
                <span className="text-2xl font-mono text-[#FDE68A] tracking-tight font-extrabold">
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            <div className="vault-glass border border-white/10 rounded-2xl p-6 shadow-xl space-y-5">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-wider text-[#D4AF37] font-mono">Statutory Control</span>
                <span className="text-xs text-slate-400 font-mono">CCPA 2022 Guidelines</span>
              </div>

              <div className="space-y-2">
                <h4 className="text-lg font-bold text-white font-poppins">Voluntary Service Charge Waiver</h4>
              ...
              <div 
                ...
                className="flex items-center justify-between p-4 rounded-xl bg-black/40 border border-white/10 cursor-pointer hover:border-[#D4AF37]/50 transition-all"
              >
                <div className="space-y-0.5">
                  <span className="text-xs font-bold text-white block">Enforce CCPA Zero-Levy</span>
                  <span className="text-[11px] text-slate-400">Exclude non-statutory service charge</span>
                </div>
                <div className={`w-12 h-6 rounded-full p-1 transition-colors ${waiveServiceCharge ? 'bg-[#D4AF37]' : 'bg-slate-700'}`}>
                  <div className={`w-4 h-4 rounded-full bg-slate-950 transition-transform ${waiveServiceCharge ? 'translate-x-6' : 'translate-x-0'}`} />
                </div>
              </div>

              {/* Real-time Savings Banner */}
              <div className="p-4 rounded-xl bg-gradient-to-br from-emerald-500/15 via-[#D4AF37]/10 to-transparent border border-emerald-500/30">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <span className="text-2xl font-mono font-extrabold text-[#FDE68A] mt-1 block">
                      ₹{netSavings.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </span>
                  </div>
                  <div className="w-10 h-10 rounded-xl bg-[#D4AF37]/20 border border-[#D4AF37]/40 flex items-center justify-center text-[#FDE68A]">
                    <ShieldCheck size={20} />
                  </div>
                </div>
            ...
            <div className="p-5 rounded-2xl vault-glass border border-white/10 text-xs text-slate-300 space-y-2">
