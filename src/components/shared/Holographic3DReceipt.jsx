      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-white/10 pb-4 mb-4">
          <div>
            ...
            <h4 className="text-lg font-bold text-white font-poppins">The Obsidian Grill</h4>
            <p className="text-xs text-slate-400 font-mono">GSTIN: 27AABCT9981Q1Z4 • Invoice #8841</p>
          </div>
          <span className="px-2.5 py-1 rounded-full text-[10px] font-mono font-bold bg-amber-500/15 border border-amber-400/30 text-amber-300">
            AUDIT IN PROGRESS
          </span>
        </div>
        ...
        <div className="space-y-2.5 text-xs font-mono mb-4">
          ...
          <div className="flex justify-between text-slate-300">
            ...
            <span className="text-white font-bold">₹7,600.00</span>
          </div>
          <div className="flex justify-between text-slate-300">
        ...
        {/* Breakdown Box */}
        <div className="rounded-xl bg-black/40 border border-white/10 p-3.5 space-y-2 mb-4 font-mono text-xs">
          <div className="flex justify-between text-slate-400">
            <span>Subtotal (Food & Drinks)</span>
            <span className="text-white font-semibold">₹{subtotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
          </div>

          {/* Service Charge Line with Discrepancy Highlight */}
          <div className={`flex items-center justify-between p-2 rounded-lg transition-colors ${
            waiveServiceCharge ? 'bg-rose-500/10 border border-rose-500/30' : 'bg-white/5'
          }`}>
            <div className="flex items-center gap-1.5">
              <AlertCircle size={14} className={waiveServiceCharge ? 'text-rose-400' : 'text-slate-400'} />
              <div>
                <span className={waiveServiceCharge ? 'text-rose-300 font-bold' : 'text-slate-300'}>
                  Service Charge (10%)
                </span>
            ...
            <div className="text-right">
              <span className={`font-bold ${waiveServiceCharge ? 'text-rose-400 line-through' : 'text-white'}`}>
                ₹{serviceCharge.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
              </span>
              {waiveServiceCharge && (
                <div className="text-[9px] text-emerald-400 font-bold">WAIVED (₹0.00)</div>
              )}
            </div>
          </div>
          ...
          <div className="pt-2 border-t border-white/10 flex justify-between items-baseline">
            <span className="text-sm font-bold text-white font-poppins">Total Net Payable</span>
            <span className="text-base font-extrabold font-mono text-[#FDE68A]">
        ...
        <div className="flex items-center justify-between pt-1">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1.5 cursor-pointer transition-all ${
              waiveServiceCharge 
                ? 'bg-[#D4AF37] text-slate-950 hover:bg-[#FDE68A] shadow-lg shadow-[#D4AF37]/20 font-bold'
